    ...
    <div class="modal" :class="{ 'is-active': showModal }">
      ...
      <div class="modal-card has-text-left">
        <header class="modal-card-head has-background-grey-darker">
          <p class="modal-card-title has-text-white"><span class="name is-size-4">{{ name }}</span><span v-if="credit" class="credit is-size-6-tablet is-size-7-mobile" v-html="credit"></span></p>
...
export default {
  ...
  computed: {
    setThumb() {
      return `${this.image}?w=256&h=256&fit=thumb`
    },
    setImage() {
      return `${this.image}?w=600&h=400&fit=pad`
    }
  },
  methods: {
    buttonClick() {
